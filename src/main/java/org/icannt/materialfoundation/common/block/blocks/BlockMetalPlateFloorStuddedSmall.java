 ...
 */
public class BlockMetalPlateFloorStuddedSmall extends BlockVariantBase {

    private static final PropertyEnum<EnumMetalType> VARIANT = PropertyEnum.create("metal", EnumMetalType.class);

    public BlockMetalPlateFloorStuddedSmall() {
        super(Material.IRON, MapColor.IRON, "metal_plate_floor_studded_small");
    }
    ...
    public void initClient() {
        ModelLoader.setCustomMeshDefinition(Item.getItemFromBlock(this), stack -> {
            ...
            Map<IBlockState, ModelResourceLocation> variants = dispatcher.getBlockModelShapes().getBlockStateMapper().getVariants(BlockMetalPlateFloorStuddedSmall.this);
            return variants.get(BlockMetalPlateFloorStuddedSmall.this.getDefaultState().withProperty(VARIANT, metal));
        });
    }
}
