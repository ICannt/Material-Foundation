 ...
 */
public class BlockMetalPaintedPlateFloorStuddedSmall extends BlockVariantBase {

    private static final PropertyEnum<EnumMetalPaintedType> VARIANT = PropertyEnum.create("metal", EnumMetalPaintedType.class);

    public BlockMetalPaintedPlateFloorStuddedSmall() {
        super(Material.IRON, MapColor.IRON, "metal_painted_plate_floor_studded_small");
    }
    ...
    public void initClient() {
        ModelLoader.setCustomMeshDefinition(Item.getItemFromBlock(this), stack -> {
            ...
            Map<IBlockState, ModelResourceLocation> variants = dispatcher.getBlockModelShapes().getBlockStateMapper().getVariants(BlockMetalPaintedPlateFloorStuddedSmall.this);
            return variants.get(BlockMetalPaintedPlateFloorStuddedSmall.this.getDefaultState().withProperty(VARIANT, metal));
        });
    }
}
