 ...
 */
public class BlockMetalPlateFloorStuddedLarge extends BlockVariantBase {

    private static final PropertyEnum<EnumMetalType> VARIANT = PropertyEnum.create("metal", EnumMetalType.class);

    public BlockMetalPlateFloorStuddedLarge() {
        super(Material.IRON, MapColor.IRON, "metal_plate_floor_studded_large");
    }
    ...
    public void initClient() {
        ModelLoader.setCustomMeshDefinition(Item.getItemFromBlock(this), stack -> {
            ...
            Map<IBlockState, ModelResourceLocation> variants = dispatcher.getBlockModelShapes().getBlockStateMapper().getVariants(BlockMetalPlateFloorStuddedLarge.this);
            return variants.get(BlockMetalPlateFloorStuddedLarge.this.getDefaultState().withProperty(VARIANT, metal));
        });
    }
}
