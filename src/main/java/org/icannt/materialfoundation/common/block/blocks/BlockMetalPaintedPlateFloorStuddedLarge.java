 ...
 */
public class BlockMetalPaintedPlateFloorStuddedLarge extends BlockVariantBase {

    private static final PropertyEnum<EnumMetalPaintedType> VARIANT = PropertyEnum.create("metal", EnumMetalPaintedType.class);

    public BlockMetalPaintedPlateFloorStuddedLarge() {
        super(Material.IRON, MapColor.IRON, "metal_painted_plate_floor_studded_large");
    }
    ...
    public void initClient() {
        ModelLoader.setCustomMeshDefinition(Item.getItemFromBlock(this), stack -> {
            ...
            Map<IBlockState, ModelResourceLocation> variants = dispatcher.getBlockModelShapes().getBlockStateMapper().getVariants(BlockMetalPaintedPlateFloorStuddedLarge.this);
            return variants.get(BlockMetalPaintedPlateFloorStuddedLarge.this.getDefaultState().withProperty(VARIANT, metal));
        });
    }
}
