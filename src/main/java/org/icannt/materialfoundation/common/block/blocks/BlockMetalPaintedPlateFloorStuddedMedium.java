 ...
 */
public class BlockMetalPaintedPlateFloorStuddedMedium extends BlockVariantBase {

    private static final PropertyEnum<EnumMetalPaintedType> VARIANT = PropertyEnum.create("metal", EnumMetalPaintedType.class);

    public BlockMetalPaintedPlateFloorStuddedMedium() {
        super(Material.IRON, MapColor.IRON, "metal_painted_plate_floor_studded_medium");
    }
    ...
    public void initClient() {
        ModelLoader.setCustomMeshDefinition(Item.getItemFromBlock(this), stack -> {
            ...
            Map<IBlockState, ModelResourceLocation> variants = dispatcher.getBlockModelShapes().getBlockStateMapper().getVariants(BlockMetalPaintedPlateFloorStuddedMedium.this);
            return variants.get(BlockMetalPaintedPlateFloorStuddedMedium.this.getDefaultState().withProperty(VARIANT, metal));
        });
    }
}
